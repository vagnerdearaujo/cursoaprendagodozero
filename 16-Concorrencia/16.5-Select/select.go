...
func main() {
	...
	/*
		...
		O correto é que para cada mensagem 2 tivessemos 2x a mensagem 1, pois o tempo de espera é
		1/2s contra 1s.
	*/
	for i := 0; i < iteracoes; i++ {
		...
		mensagem1 := <-canal1
		mensagem2 := <-canal2
		fmt.Println("Sem select: " + mensagem1)
		fmt.Println("Sem select: " + mensagem2)
	}
	...
	*/
	...
	for i := 0; i < iteracoes; i++ {
		...
		case mensagem1 := <-canal1:
			fmt.Println("Com select: " + mensagem1)
		case mensagem2 := <-canal2:
			fmt.Println("Com select: " + mensagem2)
		}
	}
}
...
func enviamensagem_canal(canal1, canal2 chan string, iteracoes int) {
	...
	go func(vezes int) {
		for i := 0; i < vezes; i++ {
			time.Sleep(time.Second) //Pausa de 1s
			canal2 <- "Canal 2"
		}
